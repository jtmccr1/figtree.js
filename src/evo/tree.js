...
export class Tree {
  ...
  *pseudoRerootPostOrder(node, incomingNode = null,nodeOrdering=(a,b)=>1) {
  ...
  toNewick(node = this.root) {
    function oneLiner(node) {
      return (
        (node.children
          ? `(${node.children.map((child) => oneLiner(child)).join(",")})${
              node.label ? "#" + node.label : ""
            }`
          : node.name) + (node.length ? `:${node.length}` : "")
      );
    }
  ...
  lastCommonAncestor(node1, node2) {
    ...
    const sharedAncestors = path1.filter(
      (n1) => path2.map((n2) => n2.id).indexOf(n1.id) > -1
    );
  ...
  subTree(chosenNodes) {
    const sharedNodes = [
      ...chosenNodes.map((node) => [...Tree.pathToRoot(node)]),
    ] // get all the paths to the root
      .reduce((acc, curr) => [...acc, ...curr], []) // unpack the paths
      .filter(
        (node, i, all) =>
          all.filter((x) => x === node).length === chosenNodes.length
  ...
  annotateNode(node, annotations) {
    ...
    node._annotations = {
      ...(node.annotations === undefined ? {} : node.annotations),
      ...annotations,
    };
  }
    ...
    for (let [key, addValues] of Object.entries(annotations)) {
      ...
      if (Array.isArray(addValues)) {
        ...
        if (
          ...
          addValues
            .map((v) => parseFloat(v))
            .reduce((acc, curr) => acc && Number.isInteger(curr), true)
        ) {
          type = Type.INTEGER;
        } else if (
          addValues
        ...
        if (annotation.type && annotation.type !== type) {
          if (
            (type === Type.INTEGER && annotation.type === Type.FLOAT) ||
            ...
            if (annotation.values) {
              ...
              throw Error(
                `existing values of the annotation, ${key}, in the tree is discrete.`
              );
            }
          }
        }
        ...
        for (let [key, value] of Object.entries(addValues)) {
          ...
          if (typeof value === typeof 1.0) {
            ...
            if (type === Type.DISCRETE) {
              throw Error(
                `the values of annotation, ${key}, should be all boolean or all floats`
              );
            }
            ...
            if (sum > 1.01) {
              throw Error(
                `the values of annotation, ${key}, should be probabilities of states and add to 1.0`
              );
            }
        ...
        if (annotation.type && annotation.type !== type) {
          throw Error(
            `existing values of the annotation, ${key}, in the tree is not of the same type`
          );
        }

        annotation.type = type;
        annotation.values = annotation.values
          ? [...annotation.values, addValues]
          : [addValues];
      } else {
  ...
  getClades(tipNameMap = null) {
    return this.nodeList
      .filter((n) => n.parent)
      .map((node) => node.getClade(tipNameMap));
  }
  ...
  static parseNewick(newickString, options = {}) {
    ...
    const tokens = newickString.split(
      /\s*('[^']+'|"[^"]+"|;|\(|\)|,|:|=|\[&|\]|\{|\})\s*/
    );
    let level = 0;
...
function calculateHeights(tree) {
  ...
  tree.nodeList.forEach(
    (node) => (node._height = maxRTT - tree.rootToTipLength(node))
  );
...
function calculateLengths(tree) {
  tree.nodeList.forEach(
    (node) =>
      (node._length = node.parent ? node.parent.height - node.height : 0)
  );
...
function fitchParsimony(name, node) {
  if (!node.children) {
    ...
    return Array.isArray(node.annotations[name])
  ...
  node.children.forEach((child) => {
    ...
    I =
      I === undefined
        ? childStates
        : childStates.filter((state) => I.includes(state)); // take the intersection
  });
...
function reconstructInternalStates(name, parentStates, acctran, node) {
  ...
  if (node.children) {
    ...
    node.children.forEach((child) => {
      reconstructInternalStates(name, nodeStates, acctran, child).forEach(
        (state) =>
          (stateCounts[state] = stateCounts[state]
            ? (stateCounts[state] += 1)
            : 1)
    ...
    const max = Object.entries(stateCounts).reduce((prev, current) =>
      prev[1] > current[1] ? prev : current
    )[1];
    nodeStates = Object.entries(stateCounts)
      .filter(([state, count]) => count === max)
      .map(([state, count]) => state);

    node.annotations[name] =
      nodeStates.length === 1 ? nodeStates[0] : nodeStates;
  }
...
class Node {
  ...
  set name(value) {
    if (this._tree._labelMap.has(label)) {
      throw new Error(
        `Node Names are used to key nodes and must be unique. Name ${value} already exists in the tree`
      );
    }
    if (this._children) {
      throw new Error(
        `Node Names are only allowed for external nodes.Name ${value} was added to a node with children`
      );
    }
  ...
  set label(value) {
    if (this._tree._labelMap.has(value)) {
      throw new Error(
        `Node labels are used to key nodes and must be unique. Label ${value} already exists in the tree`
      );
    }
    if (this._children === null) {
      throw new Error(
        `Node Labels are only allowed for internals  nodes. label ${value} was added to a node without children`
      );
    }
  ...
  set annotations(value) {
    throw new Error(
      "node annotations can not be set this way. The tree needs to know about annotation types. Use tree.annotateNode to set annotations"
    );
  }
  /**
  ...
  addChild(node) {
    if (node.tree !== this.tree) {
      throw new Error(
        "Trying to add a node as a child but the child node and parent don't not belong to the same tree!"
      );
    }
    ...
    if (this._children === null) {
      ...
      if (this.name) {
        console.warn(
          `Adding a child to a named node :${this.name}. uncertain behaviour a head. There be dragons`
        );
      }
    }
  ...
  getClade(tipNameMap = null) {
    ...
    if (!this.children) {
      ...
      if (nodeNumericId !== parseInt(nodeNumericId)) {
        throw new Error(
          "Getting clade requires tips have integer id's. If they do not please provide a map to integers keyed by the tips' names "
        );
      }
    ...
    return {
      ...
      children:
        this.children && this.children.length > 0
          ? this.children.map((child) => child.toJS())
          : null,
    };
  }
}
